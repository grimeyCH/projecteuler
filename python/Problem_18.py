...
def main():
    triangle = read_numbers()
    for x in range(len(triangle)-1,0,-1):
        for y in range(0,x):
            triangle[x-1][y] = int(triangle[x-1][y]) + int(max(triangle[x][y],triangle[x][y+1]))
    print(triangle[0][0])

def read_numbers():
    return [line.split() for line in file("problem_18_numbers.txt").readlines()]
